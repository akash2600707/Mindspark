...
async function advanceIfExpired(db:any, quizId:string, state:any){
  ...
  const endsAt=new Date(
    startedAt.getTime()+(nextQuestion.time_limit_seconds||15)*1000
  );
  ...
  const {data:fresh}=await db.from('quiz_state')
    .select('*')
    .eq('quiz_id',quizId)
    .maybeSingle();
...
export async function POST(req:Request){
  try{
    ...
    const {data:p}=await db.from('participants')
      .select('id,quiz_id,full_name,status')
      .eq(
        'participant_code',
        String(participantCode).toUpperCase()
      )
      .maybeSingle();
    ...
    let {data:state}=await db.from('quiz_state')
      .select(
        'quiz_id,status,current_question_id,question_started_at,question_ends_at'
      )
    ...
    // FIX: state may become null after advanceIfExpired()
    if(!state){
      return NextResponse.json({
        status:'WAITING',
        fullName:p.full_name
    ...
    if(state.current_question_id){
      const {data:q}=await db.from('questions')
        .select(
          'id,question_number,question_text,option_a,option_b,option_c,option_d,points,time_limit_seconds,image_url,category'
        )
        .eq('id',state.current_question_id)
        .maybeSingle();
    ...
    if(question){
      const {data:a}=await db.from('answers')
        .select('selected_option,points_awarded,is_correct')
        .eq('participant_id',p.id)
